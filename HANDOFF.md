...
- **Tier 1 — app launch: the animated "Goodfriends." wordmark.** Letters stagger in (fade + rise, left-to-right ~38ms apart), hold, reveal.
  - Mobile: `components/LaunchWordmark.tsx` (Reanimated `FadeInDown` per letter), mounted as an overlay in `app/_layout.tsx`, dismisses via `onDone` → `launchDone`. Plays once per cold start.
  - Web: inline in `App.jsx`'s `loading` branch — each glyph is a `<span class="gf-wm-letter">` with inline `animationDelay` (keyframes in `index.css`).
...
17. **Native splash is config, not JS — it bakes into the build.** `app.json`'s `expo-splash-screen` plugin now shows a **plain cream** screen (no image) so the JS `LaunchWordmark` stagger is the first wordmark (a baked mark would re-stagger and flicker). Changing it takes a `expo prebuild -p ios --clean` + rebuild (or the next EAS build) — the *currently installed* dev build still shows the old ink-mark splash until then; that's expected, not a regression. The JS launch overlay degrades fine over either splash.
