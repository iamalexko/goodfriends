...
/notifications   → app/notifications.tsx   (in-app feed; opened from the header bell)
```
...
- ✅ **Nav chrome** — `NativeTabs` (UIKit-rendered Liquid Glass tab bar) + per-tab **native Stack headers** (`BrandHeader`, iOS 26 glass). PRs #26–#29 + the native-header migration.
...
- There is **no Create tab/FAB in the bar** anymore. "+ Plan" lives in the header (`BrandHeader`) and pushes the `/create` modal.
...
### Top header — native Stack header (iOS 26 glass), all tabs

All four tabs (Home, Crew, Plans, Profile) use a real **native Stack header** — each tab is a Stack nested under it (`app/(tabs)/<tab>/_layout.tsx` spreads `brandStackScreenOptions` + `<tab>/index.tsx` is the screen). The old custom `AppHeader` (BlurView) is **deleted**.

> **Key gotcha — render the brand row as `headerTitle`, NOT `headerLeft`/`headerRight`.** iOS 26 capsule-wraps left/right bar-button items in glass "shared background" pills and **react-native-screens 4.16 has no opt-out**, which put an unwanted pill behind the wordmark and grouped `+ Plan`+bell into one capsule. The title view isn't wrapped, so `components/BrandHeader.tsx` → `BrandHeaderRow` (full-width `width−32` row) renders: the bare PJS800 "Goodfriends." `<Text>` (no glass behind it) + two **separate** glass buttons (gap 8) — `PlanButton` (black liquid-glass pill: `GlassView` `tintColor="#000000"` + `colorScheme="dark"` + inner `rgba(0,0,0,0.55)` darken fill, white label; solid-ink `#111` fallback on iOS 18) and `BellButton` (its own `GlassView` circle + unread badge; brightened with `tintColor="#FFFFFF"` + `colorScheme="light"` + a hairline edge; flat-white fallback). Both guarded by `isLiquidGlassAvailable()`. Bar is `headerTransparent: true`, no `headerStyle` bg → no solid color; iOS renders its own glass. Scroll-root screens (Home/Crew/Profile) make the screen's direct first child a `ScrollView` with `contentInsetAdjustmentBehavior="automatic"`; **Plans** has a FIXED title+tabs header above a pager so it pads that with `useHeaderHeight()` (`@react-navigation/elements`). `(tabs)/_layout.tsx`'s `ThemeProvider` base is `useColorScheme()`-matched (dark-mode glass-flicker fix). **No scroll animation.** **Pending real-device iOS 26 validation** — glass doesn't render in Expo Go or meaningfully in the Simulator. Full rationale in DESIGN_SYSTEM §3.
...
- **Floating glass composer (Claude pattern)** — the Moments composer is a **floating iOS 26 liquid-glass pill** (`ComposerSurface`: `GlassView` guarded by `isLiquidGlassAvailable()`, solid-cream + hairline + shadow fallback), **absolutely positioned** (left/right 12, above the bottom safe area) so the feed scrolls behind it; a `LinearGradient` scrim (transparent→`rgba(255,251,245,0.55)`) keeps it legible. The **text row is the anchor**; a ~54px photo thumbnail (with ×) inserts **above** it inside the same container (placeholder swaps "Add a moment…"→"Add a caption…"). The screen root is a `View`; the `KeyboardAvoidingView` now wraps **only the ScrollView**, and the composer (absolute sibling) is lifted above the keyboard via reanimated `useAnimatedKeyboard` (`composerKbStyle`, translate by `keyboard.height − insets.bottom`). Scroll content pads `insets.bottom + 104` to clear it. Full rules in DESIGN_SYSTEM §3. (Pending real-device iOS 26 glass validation; sim shows layout/states/fallback only.)
...
**Keyboard handling — two patterns by input position.** For an input that lives *mid-scroll*, `automaticallyAdjustKeyboardInsets` on the ScrollView (iOS) + `keyboardShouldPersistTaps="handled"` is simplest. For a **floating/absolute** input (the detail page's glass composer), it can't ride `KeyboardAvoidingView` padding — drive it with reanimated `useAnimatedKeyboard` (translate by `keyboard.height − insets.bottom`) and keep `KeyboardAvoidingView` wrapping only the ScrollView. Don't combine the two on the same element.

**Frosted glass needs rich content behind it.** A `BlurView` over the mostly-flat `#FFFBF5` cream reads muddy/washed-out, not premium (tried it on the RSVP selected tile — looked grey and dirty). Over flat backgrounds use a **solid soft tint + a clean border/glow** instead of blur. `BlurView` only pays off over an image or busy content (photo lightboxes, image/busy backgrounds).
...
18. **EAS builds DON'T see your local `.env` — set `EXPO_PUBLIC_*` on EAS.** TestFlight build #3 crashed on launch with `Error: supabaseUrl is required.` (abort/signal 6, thrown from `createClient` while expo-router built the route tree) because `EXPO_PUBLIC_SUPABASE_URL` / `_ANON_KEY` live in a gitignored local `.env` that the EAS cloud never gets — so `createClient` got `undefined`. Dev works (Expo loads `.env`); EAS doesn't. Fix: set them as EAS env vars — `eas env:create --environment production --environment preview --name EXPO_PUBLIC_SUPABASE_URL --value … --visibility plaintext` (and the anon key). They're the public URL + publishable key (RLS-protected, already in the shipped bundle), so plaintext on EAS is fine; kept out of the public repo. Any new `EXPO_PUBLIC_*` the app reads must be added on EAS too.
19. **Capture a release/TestFlight crash from a paired iPhone, no Xcode UI:** `xcrun devicectl list devices` to get the id, then `xcrun devicectl device process launch --console --terminate-existing --device <id> com.goodfriends.crew` — it streams the app's console and prints the JS error/exception + "App terminated due to signal N" on a launch crash. (On-device alt: Settings → Privacy & Security → Analytics & Improvements → Analytics Data → the `Goodfriends-…ips` file.)
