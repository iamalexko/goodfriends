...
- **Buttons match the original, bare:** `+ Plan` is the solid-ink CTA; the bell is a separate **flat `rgba(0,0,0,0.05)` circle** (no per-button `GlassView` — it rendered clear on the sim and the system capsule was the problem). The native glass lives in the **bar + the NativeTabs bar**, not the buttons. Badge unchanged.
- **Bar, not transparent:** no `headerTransparent` — the bar keeps the iOS 26 glass background; content lays out below it.
