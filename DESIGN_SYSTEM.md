...
- **Each button carries its own liquid glass.** `+ Plan` and the bell are **separate** buttons, each its own `GlassView` (guard: `isLiquidGlassAvailable()`; fallback = the solid-ink `+ Plan` pill + a flat `rgba(0,0,0,0.05)` bell circle on iOS 18 / unsupported). They're distinct elements in the title row (gap 8) — NOT grouped, NOT the system shared-background capsule. The `+ Plan` label/icon go **ink `#111`** on glass (readable over cream). Badge unchanged.
