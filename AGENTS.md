...
_This file and `CLAUDE.md` are kept in sync — they carry the same instructions for different toolchains (Codex/Cursor/others read `AGENTS.md`; Claude Code reads `CLAUDE.md`). **If you edit one, edit the other.** All depth lives in the three docs they point to, so changes here should be rare._
